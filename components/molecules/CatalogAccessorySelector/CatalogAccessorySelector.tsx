...
export const CatalogAccessorySelector = ({
      ...
      response.items.forEach((acc) => {
        if (acc.variants && acc.variants.length > 0) {
          acc.variants.forEach((variant) => {
            flattenedItems.push({
              internalId: "", // Placeholder, not used here
              accessoryId: acc.id,
  ...
  const handleSelect = (item: AutoCompleteItem) => {
    const selected = item.originalItem as Omit<CatalogAccessoryItem, "internalId">;
    if (selected) {
      const itemWithId = {
        ...selected,
        internalId: Math.random().toString(36).substr(2, 9),
      };
      onItemSelect(itemWithId as CatalogAccessoryItem);
    }
    setSearchQuery("");
  };
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-4">
          {selectedItems.map((item) => (
            ...
            >
              ...
              <div className="w-full aspect-square relative mb-2 rounded overflow-hidden">
                <ImageWithFallback
                  src={item.imageUrl}
                  alt={item.name}
